...
- **目录：**
  ...
  - `src/lib/` — 基础设施层：业务无关的工具、provider 工厂与客户端（`db`、`logger`、`utils`、`rate-limit`、`region`、`auth`、`email`、`storage`、`stripe` 等）
  - `src/services/` — 业务服务层：各业务域的逻辑、Server Actions 与 cron（`account`、`admin`、`audit`、`billing`、`data-export`、`jobs`、`orgs`、`sso`、`webauthn`、`webhooks`）
...
- **模块系统：** 源码统一使用 ES Module（`import` / `export`）。禁止在业务代码中出现 `require()` / `module.exports`。脚本文件统一用 `.ts` + `tsx` 执行。唯一例外：必须通过 Node `--require` 预加载的文件，因 `--require` 不支持 TypeScript 且必须同步执行，允许使用 `.cjs` 并显式标注原因。
...
- **不要做：**
  ...
  - 用 `npm` / `yarn` 发命令
  - 将业务域逻辑（含 Server Actions）放入 `src/lib/`，应放入 `src/services/`
  - 在业务代码中使用 `require()` / `module.exports`（一律用 ESM）
  - 新增脚本用裸 `.js` 文件（用 `.ts` + `tsx`；必须 CJS 预加载时才用 `.cjs`）
- **要做：**
  - 修改前先读相关现有代码
  - UI 任务先看设计契约（第 3 节）
