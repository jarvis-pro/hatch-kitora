...
| 实现 commits | PR-1 — `feat: RFC 0002 PR-1 实现 Active Sessions（DeviceSession + sid claim + 列表与撤销）`<br>PR-2 — `feat: RFC 0002 PR-2 实现 2FA（TOTP + backup codes + 登录拦截 + admin reset）`<br>PR-3 — `feat: RFC 0002 PR-3 实现 GDPR 数据导出（异步 zip + cron + 鉴权下载 + user/org 双维度）`<br>PR-4 — `feat: RFC 0002 PR-4 实现注销宽限期 30 天 + Org 强制 2FA` |
