...
| 状态     | **Implemented**（2026-04-26，工程层 v0.7.0；ICP 备案 / 商户开户 / Aliyun 实名仍在 PR-0 阶段，非代码）                                                                                             |
| -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 作者     | Jarvis                                                                                                                                                                                            |
| 创建于   | 2026-04-26                                                                                                                                                                                        |
| 影响版本 | 0.6.0 → 0.7.0（非破坏性，新增 provider 实现 + 部署 pipeline + 加 3 列至 Subscription + 新增 BillingEvent 表）                                                                                     |
...
### 6.1 Subscription 模型扩展

`Subscription` 加三个字段并把 `stripeSubscriptionId` 放开为可空（CN 路径不使用这个字段）：

```prisma
model Subscription {
  // ... 现有字段
  // 'stripe' | 'alipay' | 'wechat'，stamp 在创建时，不可变。
  provider             String  @default("stripe")
  // CN 行不带 Stripe sub id；改为 nullable（Postgres UNIQUE 仍生效，多个 NULL 不互相冲突）。
  stripeSubscriptionId String? @unique
  // 支付宝免密协议号 / 微信周期扣款合同号；Stripe 路径下为 null。
...
应用层不变量（Prisma 表达不出来，靠代码守护）：

1. `provider = 'stripe'` ⇒ `stripeSubscriptionId NOT NULL ∧ cnAgreementId IS NULL`
2. `provider in ('alipay','wechat')` ⇒ `cnAgreementId NOT NULL ∧ stripeSubscriptionId IS NULL`（注：WeChat Native 单笔支付路径 `cnAgreementId` 暂留 null，待 papay 周期扣款 follow-up）
3. 行的 `region`（透过 Organization 反查）决定哪种 provider 合法——Stripe 在 GLOBAL/EU，Alipay/WeChat 在 CN。Provider factory 是唯一的选择点，webhook 路由拒绝跨 provider 事件。

迁移：`prisma/migrations/20260601000000_add_cn_subscription_fields/`，纯加列 + 索引 + relax NOT NULL，秒级。

### 6.2 BillingEvent 新表（仅 Alipay + WeChat）

> **设计偏离**（PR-3 实施时发现）：仓库里**没有** `BillingEvent` 表，只有 `StripeEvent`，且后者被 `/admin/stripe-events` 列表 / groupBy / nav 共 4 处引用。重命名 `StripeEvent → BillingEvent` 拉爆 admin UI 层，零功能收益。改为：保留 `StripeEvent` 不动，新增独立的 `BillingEvent` 表，**仅服务 Alipay + WeChat**。
...
迁移：纯加表，零回填，秒级。`StripeEvent` 表结构与读写路径**零变更**。两个 Stripe / CN 入站 webhook 各自独立做幂等去重，互不干扰。后续若有动力做统一可再立项 follow-up RFC，但「合并表」本身不在交付红线上。
...
> **代码部分**已完成；**生产部署** 仍在 PR-0 备案阶段（ICP / 公安部 / Aliyun 实名 / 域名注册 / 商户开户），属于 RFC 范围内但非代码型的工作，里程碑外列。

工程交付清单（按 PR 排）：

- **PR-2**（邮件 + 对象存储 provider）—— `src/lib/email/aliyun-direct-mail.ts`、`src/lib/storage/aliyun-oss.ts`、`src/lib/email/send.ts`（region 分支）、`src/lib/storage/index.ts`（region-aware facade）、`src/lib/region/providers.ts`（拿掉 not-implemented）、env 7 项 + 3 个 SDK 依赖。
- **PR-3**（schema + 支付）—— `prisma/migrations/20260601000000_add_cn_subscription_fields/`、`prisma/migrations/20260601001000_add_billing_event_table/`、`src/services/billing/provider/{alipay,wechat,cn-price-config}.ts`、`src/app/api/billing/{alipay,wechat}/notify/route.ts`、env 9 项 + 2 个 SDK 依赖。
- **PR-4**（限流 + 出境闸）—— `src/lib/rate-limit.ts`（ioredis + 自写 ZSET 滑窗）、`scripts/audit-egress.ts`（CI 阻断）、`pnpm egress:check` npm script、`ALIYUN_REDIS_URL` env、`ioredis` 依赖。
- **PR-5**（合规入口 + 部署 pipeline + 文档收尾）—— `src/app/[locale]/(marketing)/legal/data-rights/page.tsx`（PIPL §44 入口）、`SiteFooter` CN 模式追加 data-rights 链接、`messages/{en,zh}.json` 国际化、`.github/workflows/deploy-cn.yml`（OIDC + ACR + ACK rollout + 回滚）、`docs/deploy/cn.md` 收尾。
- **未交付**（RFC §1 已明确不在范围内的非目标，体例对齐 RFC 0001–0005）—— PR-1 Terraform IaC（基础设施代码化已立项但未完成；ACK / RDS / Redis / OSS / SLS / SLB / WAF / KMS 的 `infra/aliyun/*.tf` 留作 follow-up）；Sentry self-hosted 的实际节点搭建；Prometheus / Grafana / 钉钉告警的 ACK 内部部署；Smoke e2e Playwright 套件（需真 CN 集群才能跑）。
- **决策回填**（§12 待评审项）：
  - ✅ Alipay + WeChat **双开**（PR-3 已落两套 provider）
  - ✅ WeChat **仅 Native** 路径（PR-3 不做 JSAPI / H5）
  - ✅ `audit-egress` 阻断级别 **CI red**，但仅在 `KITORA_REGION=CN` 或 `--strict` 时生效（PR-4）
  - ✅ Sentry **self-hosted**（部署 pipeline 准备就位，节点搭建非代码）
  - ⏳ 运营主体城市 / 人民币定价档位 / PIPL 同意书法务签发 / OSS BYOK —— 留给 PR-0 备案阶段拍板，非代码
- **首日观测指标**（生产上线后回填）：订单成功率 / 邮件投递成功率 / OSS 写入 P95 延迟 / rate-limit 命中率 / `audit-egress` 报告条数。
