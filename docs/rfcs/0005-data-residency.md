...
- [ ] **region 枚举值** —— `GLOBAL` / `CN` / `EU`。备选「`US` / `CN` / `EU`」更符合"region = 地理位置"语义，但 `GLOBAL` 更准确表达"非中国非欧洲的默认 region"。**待定**。
- [ ] **现有 `REGION` 环境变量的兼容期** —— 1 个版本（v0.6 + v0.7 都接受 `REGION`，v0.8 移除）。**待定**。
- [ ] **跨 region 同邮箱注册** —— 允许，独立账号体系。备选「禁止 + 强制单一注册」UX 差且无技术必要。**已倾向允许**。
- [ ] **Org 邀请校验时机** —— UI 输入邮箱时实时异步查 vs server action 提交时校验。后者更简单但 UX 稍差。**已倾向 server action 校验**。
- [ ] **AuditLog region 索引** —— `(region, createdAt)` 复合索引。监管报表按 region 切片是高频查询。**已倾向加**。
- [ ] **CN 部署交付 RFC** —— 单独立项 RFC 0006，本 RFC 不交付实际 CN 部署。**已倾向单独立项**。
- [ ] **EU region 启用时机** —— 不在路线图中（"加分项"非"硬门槛"）。占位 enum 即可。**已倾向占位**。
